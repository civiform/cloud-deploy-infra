...
export SENDER_EMAIL_ADDRESS="jhummel@google.com"
...
export STAGING_PROGRAM_ADMIN_NOTIFICATION_MAILING_LIST="jhummel@google.com"

# REQUIRED
# The email address that receives a notifications email each time an applicant
# submits an application to a program in the staging environments, instead of
# sending it to the trusted intermediary's email, as would happen in prod.
export STAGING_TI_NOTIFICATION_MAILING_LIST="jhummel@google.com"

# REQUIRED
# The email address that receives a notifications email each time an applicant
# submits an application to a program in the staging environments, instead of
# sending it to the applicant's email, as would happen in prod.
export STAGING_APPLICANT_NOTIFICATION_MAILING_LIST="jhummel@google.com"

# REQUIRED
# The domain name for this CiviForm deployment, including the protocol. 
# E.g. "https://civiform.seattle.gov"
export BASE_URL="https://jhummel-dev.civiform.dev"

#TODO:
# Play framework accepts the value if I directly set SUPPORTED_LANGUAGES.0=am in the compose yaml file
# However, terraform complains about SUPPORTED_LANGUAGES.1 being invalid if I 
# set it in app.tf
# Setting another variable (SUPPORTED_LANGUAGES0) in app.tf and then setting 
# SUPPORTED _LANGUAGES.0 in the compose file does not work (it says the variable is empty)

# Find way to pass the variable to application.conf
# Tests for pything code
# Tests for the set of supported languages
# Code cleanup

# OPTIONAL
# When set defines which languages are supported for applicants.
# Defaults to [ en-US, am, zh-TW, ko, so, es-US, tl , vi ]
export CIVIFORM_SUPPORTED_LANGUAGES="en-US, am, zh-TW"
...
export APP_PREFIX="jhummel-dev" # max 19 chars, only numbers, letters, dashes, and underscores
...
export SSL_CERTIFICATE_ARN="arn:aws:acm:us-east-1:857661394044:certificate/51d53eb5-9543-42ca-bd53-2976bf7eb647"
...
# OPTIONAL
# The type of OIDC flow o execute, and how the data is encoded.
