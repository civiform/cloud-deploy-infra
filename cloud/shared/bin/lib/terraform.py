...
def force_unlock(config_loader: ConfigLoader, 
                 lock_id: str,
...
def perform_init(config_loader: ConfigLoader,
                 terraform_template_dir: Optional[str] = None,
                 upgrade: bool = True):
    if not terraform_template_dir:
        terraform_template_dir = config_loader.get_template_dir()
...
def capture_stderr(cmd):
    popen = subprocess.Popen(shlex.split(cmd), stderr=subprocess.PIPE, bufsize=1, universal_newlines=True)
    try:
...
def perform_apply(
    ...
    if exit_code:
        if "Error acquiring the state lock" in output:
            ...
            match = re.search(r'ID:\s+([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})',output)
