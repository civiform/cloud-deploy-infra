...
class Tenant():
    ...
    def _kubectl_apply_server_env_vars(self):
        temp = self._write_tempfile(".yaml", yaml.dump({
            'apiVersion': 'v1',
            'kind': 'ConfigMap',
            'metadata': {'namespace': self.tenant_namespace, 'name': self.env_var_config_map_name},
    ...
    def _kubectl_apply_server_secret_env_vars(self):
        temp = self._write_tempfile(".yaml", yaml.dump({
            'apiVersion': 'v1',
            'kind': 'Secret',
            'metadata': {'namespace': self.tenant_namespace, 'name': self.env_var_secrets_name},
    ...
    def _helm_write_values(self):
        return self._write_tempfile(".yaml", yaml.dump({
