...
FROM eclipse-temurin:17.0.4_8-jre-alpine as amd64
FROM bellsoft/liberica-openjre-alpine:11.0.16-8 as arm64

FROM ${TARGETARCH}

COPY --from=hashicorp/terraform:1.3.1 /bin/terraform /usr/local/bin/
COPY --from=amazon/aws-cli:2.8.0 /usr/local /usr/local
COPY --from=amazon/aws-cli:2.8.0 /aws /aws
