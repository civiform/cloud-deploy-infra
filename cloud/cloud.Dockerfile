...
FROM bellsoft/liberica-openjre-debian:17.0.4-8 as arm64

FROM ${TARGETARCH}

COPY --from=hashicorp/terraform:1.3.2 /bin/terraform /usr/local/bin/
COPY --from=amazon/aws-cli:2.8.2 /usr/local /usr/local
