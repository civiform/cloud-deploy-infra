...
COPY --from=hashicorp/terraform:1.5.5 /bin/terraform /usr/local/bin/
COPY --from=amazon/aws-cli:2.13.10 /usr/local /usr/local
COPY --from=amazon/aws-cli:2.13.10 /aws /aws
