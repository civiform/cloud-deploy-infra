...
FROM eclipse-temurin:11.0.23_9-jre as amd64
FROM bellsoft/liberica-openjre-debian:11.0.19-7 as arm64

FROM ${TARGETARCH}

COPY --from=hashicorp/terraform:1.9.1 /bin/terraform /usr/local/bin/
COPY --from=amazon/aws-cli:2.17.9 /usr/local /usr/local
COPY --from=amazon/aws-cli:2.17.9 /aws /aws
