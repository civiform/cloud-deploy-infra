locals {
  ...
  app_settings = merge({
    ...
    DB_JDBC_STRING = "jdbc:postgresql://${locals.fqdn}:5432/postgres?user=psqladmin&password=${data.azurerm_key_vault_secret.postgres_password.value}&sslmode=require"
