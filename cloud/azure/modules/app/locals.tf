locals {
  ...
  app_settings = merge({
    ...
    AWS_ACCESS_KEY_ID     = var.email_provider == "aws-ses" ? data.azurerm_key_vault_secret.aws_access_key_id[0].value : "foo"
    AWS_SECRET_ACCESS_KEY = var.email_provider == "aws-ses" ? data.azurerm_key_vault_secret.aws_secret_access_token[0].value : "bar"
