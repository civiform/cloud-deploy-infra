locals {
  ...
  app_settings = merge({
    ...
    DB_JDBC_STRING = "jdbc:postgresql://${locals.postgres_private_link}:5432/postgres?user=${azurerm_postgresql_flexible_server.civiform.administrator_login}&password=${data.azurerm_key_vault_secret.postgres_password.value}&sslmode=require"
